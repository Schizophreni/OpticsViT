...
class SpeckleINR(nn.Module):
    def __init__(self, feat_dim, hidden=128, num_freqs=6):
        ...
        # self.mlp = nn.Sequential(
        #     nn.LayerNorm(feat_dim + num_freqs*4),
        #     SIREN([256, 256, 256, 256], 192, 1, w0=1.0, w0_initial=30, initializer='siren'),
        #     nn.Sigmoid()
        # )
...
class OpticsViTINR(nn.Module):
    def __init__(self, image_size=50, patch_size=5, enc_depth=6, dec_depth=6, dim=512, heads=8, dim_head=64,
                 mlp_dim=2048, in_channels=2, out_channels=1, act=nn.Sigmoid, out_dim=512, use_learnable_pos=False, num_reg=0, drop_path_rate=0.0):
        super().__init__()
        """
        ...
        self.patch_embed = nn.Conv2d(in_channels, dim, kernel_size=patch_size, stride=patch_size, bias=False) # nn.Sequential(
        #     nn.Conv2d(in_channels, 64, kernel_size=3, padding=1),
        #     nn.GELU(),
        #     nn.Conv2d(64, 128, kernel_size=3, stride=1, padding=1),
        #     nn.GELU(),
        #     nn.Conv2d(128, dim, kernel_size=patch_size, stride=patch_size)
        # )
        # nn.Conv2d(in_channels, dim, kernel_size=(patch_size, patch_size), stride=(patch_size, patch_size))
        # if self.in_channels == 2:
            # self.imag_patch_embed = nn.Conv2d(1, dim, kernel_size=(patch_size, patch_size), stride=(patch_size, patch_size))
            # self.embed = nn.Conv2d(dim*2, dim, 1, 1, 0)
        ...
        self.decoder = nn.Sequential(
            ...
            nn.Linear(out_dim, out_channels*self.patch_size * self.patch_size),
            # act()
        )

        self.out_embedding = nn.Sequential(
            nn.Conv2d(out_channels, 96, kernel_size=1, stride=1, padding=0),
            nn.GELU(),
        )

        self.inr = SpeckleINR(feat_dim=96, hidden=192, num_freqs=12)
    ...
    def unpatchify(self, x):
        ...
        x: [b, c, h, w] --> [b, l, d]
        """
        x = rearrange(x, 'b (h w) (p q c) -> b c (h p) (w q)', h=self.image_size//self.patch_size, p=self.patch_size, c=self.out_channels)
        return x
    
    def forward_features(self, x, scale=4):
        b = x.shape[0]
        # if self.in_channels == 1:
        x = self.patch_embed(x)
        # else:
        #     x_real, x_imag = self.patch_embed(x[:, [0]]), self.imag_patch_embed(x[:, [1]])
        ...
        out_inr = self.inr.forward(feat=feat_embed, scale=scale)
        return out, out_inr
