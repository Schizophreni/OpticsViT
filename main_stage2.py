...
def mix_pat_noise(noisy_pat, emnist):
    ...
    alpha = BetaSampling(batch_size=noisy_pat.shape[0], device=noisy_pat.device, alpha=1.0)
    # print(scale)
    # print(alpha)
...
def get_args():
    ...
    parser.add_argument("--pat_size", type=int, default=50, help='pat size')
...
def train_one_epoch(model_forward, model, dataloader, optimizer, lr_scheduler, scaler,
    ...
    for i, (signal, pat, _, emnist) in enumerate(dataloader):
        ...
        # comb_inp_pred, _ = model(comb_emnist)
        if i == 0:
            torchvision.utils.save_image(comb_emnist[:16], os.path.join("preds/stage2", "comb_emnist.png"))
        _, emnist_recon = model_forward(emnist_inp_pred, scale=3)
        # _, comb_recon = model_forward(comb_inp_pred, scale=3) 
        mask = (emnist > 0.01).float().detach()
        loss_emnist = weight_l1(emnist_recon, emnist, mask) + weight_l1(emnist_recon, emnist, 1-mask) # + 0.1*l2_criterion(comb_recon, comb_emnist)
            # loss_emnist = l2_criterion(emnist_recon, emnist) # + 0.1*l2_criterion(comb_recon, comb_emnist)
...
def validate(model_forward, model, val_loader, device):
    ...
    with torch.no_grad():
        for idx, (signal, pat, pat_name, emnist) in tqdm(enumerate(val_loader), ncols=60, desc="Validating"):
            ...
            _, pat_pred = model_forward(signal_pred, scale=3)
            emnist_inp_pred, _ = model(emnist)
            ...
            if idx < 10:
                torchvision.utils.save_image(tensor2phase(signal).cpu(), os.path.join("preds/stage2", "inp_{}.png".format(idx+1)))
...
def main(args):
    ...
    save_path = os.path.join(args.save_path, "stage2_winterpo")
    ...
    model_forward = OpticsViTINR(image_size=args.phase_size, patch_size=5, enc_depth=4, dec_depth=4, heads=8, dim_head=32, dim=256, 
                      mlp_dim=int(256*8/3), in_channels=2, out_channels=1, act=torch.nn.Sigmoid, out_dim=384, use_learnable_pos=False, num_reg=0, drop_path_rate=0.0)
    ...
    lr_scheduler = WarmupCosineLR(optimizer, warmup_iters=1500, total_iters=args.num_epochs*38000//args.batch_size)
    scaler = GradScaler()
    # load forward model
