...
def train_one_epoch(model, dataloader, optimizer, lr_scheduler, scaler,
                    device, epoch, print_interval=10, writer=None, log_file=None, args=None):
    model.train()
    running_loss = 0.0
    running_loss_inr = 0.0
    # importance = Image.open("datasets/std.png")
    # importance = img_crop(importance).unsqueeze(0).cuda()
    # importance = (1 + importance)*0.5
    ...
    with torch.no_grad():
        for idx, (signal, pat, pat_name, _) in tqdm(enumerate(val_loader), ncols=60, desc="Validating"):
            ...
            if idx < 10:
                ...
                torchvision.utils.save_image(tmp.cpu(), os.path.join("preds/phase", "inp_{}.png".format(idx+1)))
...
def main(args):
    ...
    # save key files
    shutil.copy("layers/vit_inr.py", os.path.join(args.save_path, "model.py"))
    shutil.copy("datasets/Phase_dataset.py", os.path.join(args.save_path, "dataset.py"))
    shutil.copy("main_stage1_simulation.py", os.path.join(args.save_path, "train.py"))
    shutil.copy("run.sh", os.path.join(args.save_path, "run.sh"))
    ...
    for epoch in range(start_epoch, args.num_epochs):
        ...
        train_one_epoch(model, train_loader, optimizer, lr_scheduler, scaler, device, epoch, print_interval=args.print_interval,
                        writer=writer, log_file=log_file, args=args)
        # save checkpoint
        torch.save({
            "epoch": epoch + 1,
        ...
        # validate
        psnr_mean = validate(model, val_loader, device, args=args)
