...
class OpticsViT(nn.Module):
    def __init__(self, image_size, patch_size, dim, depth, heads, mlp_dim, channels=1, dim_head=64, depth_pred=2, mask_size=32) -> None:
        super().__init__()
        """
        ...
        self.decoder = nn.Sequential(
            ...
            nn.LayerNorm(dim),
            nn.Linear(dim, 2*patch_height * patch_width)
        )
    ...
    def unpatchify(self, x):
        ...
        x: [b, c, h, w] --> [b, l, d]
        """
        x = rearrange(x, 'b (h w) (p q) -> b (h p) (w q)', h=self.image_size//self.patch_size, p=self.patch_size)
        x.unsqueeze_(1)
        return x
    ...
    def forward_features(self, x):
        ...
        x = x + self.pos_embed
        x = self.encoder(x)
        image_token = self.decoder(x)
        x_real, x_imag = torch.chunk(image_token, chunks=2, dim=-1)

        x_real = self.unpatchify(x_real)
        x_imag = self.unpatchify(x_imag)
        out = torch.cat([x_real, x_imag], dim=1)
        return out
    
    def forward(self, x, interpolate=True):
        if self.training and interpolate:
            b, c, h, w = x.shape
            shuffle_index = list(range(b))
            random.shuffle(shuffle_index)
            alpha = BetaSampling(alpha=1.0, device=x.device, batch_size=b)
            comb_x = alpha * x + (1 - alpha) * x[shuffle_index]
            xs = torch.cat([x, comb_x], dim=0)
            outs = self.forward_features(xs)
            out, out_comb = outs[:b,...], outs[b:,...]
            out_phi = self.phi(torch.cat([alpha*out, (1-alpha)*out[shuffle_index]], dim=1)) + alpha * out + (1 - alpha) * out[shuffle_index]
            loss_cons = F.mse_loss(out_comb.detach(), out_phi) + F.mse_loss(out_comb, out_phi.detach())
            return out, loss_cons
        else:
