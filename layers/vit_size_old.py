...
class OpticsViT(nn.Module):
    def __init__(self, input_size=200, input_patch_size=20, enc_depth=6, output_size=50, output_patch_size=5, dec_depth=6, 
        ...
        self.decoder = nn.Sequential(
            Transformer(out_dim, depth=dec_depth, heads=heads, dim_head=out_dim//heads, mlp_dim=int(mlp_dim*out_dim/dim)),
            nn.LayerNorm(out_dim),
            nn.Linear(out_dim, out_channels*self.out_patch_size * self.out_patch_size),
        )
        
        self.conv_out = nn.Sequential(
            nn.Conv2d(out_channels, out_channels*8, kernel_size=3, stride=1, padding=1),
            nn.GELU(),
            nn.Conv2d(out_channels*8, out_channels, kernel_size=1, stride=1, padding=0),
            act()
        )
    ...
    def unpatchify(self, x):
        ...
        x = rearrange(x, 'b (h w) (p q c) -> b c (h p) (w q)', h=self.out_size//self.out_patch_size, p=self.out_patch_size, c=self.out_channels)
        return x
