...
save_amp_dir = f'newton_results/amp_{PAT_SIZE}_{MODE}'
...
inp2pat = OpticsViTINR(image_size=PHASE_SIZE, patch_size=5, enc_depth=4, dec_depth=4, heads=8, dim_head=32, dim=256, 
                      mlp_dim=int(256*8/3), in_channels=1, out_channels=1, act=torch.nn.Sigmoid, out_dim=384, pat_size=PAT_SIZE, drop_path_rate=0.0).cuda()

ckp = torch.load('checkpoints/4f_comb_amps_20k_experiment/stage1/best.pth', weights_only=False)
...
for idx in range(100): 
    ...
    tv_utils.save_image(phase_radi, f"{save_amp_dir}/emnist_{idx}.png")
    phase = phase_radi[0, 0].detach().cpu() # (phase_radi * 1023)[0,0].detach().cpu().numpy().astype('uint16')
    np.save(f"{save_amp_dir}/emnist_{idx}.npy", phase)
