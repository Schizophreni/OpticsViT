...
# Cache Gaussian kernels keyed by (kernel_size, sigma, device, dtype)
_gauss_kernel_cache = {}

def _make_gauss_kernel(kernel_size, sigma, device, dtype):
...
def gaussian_torch(x, sigma=1.0):
    """CUDA-accelerated 2D Gaussian filter for [B, C, H, W] tensors.

    Low-pass filter that smooths high-frequency speckle while preserving
    spot envelope shape.
    """
    if sigma <= 0:
        return x
    ...
    kernel = _make_gauss_kernel(kernel_size, sigma, x.device, x.dtype)
    B, C, H, W = x.shape
    # conv2d expects [out_c, in_c, kH, kW]; apply same kernel per channel
    out = F.conv2d(x, kernel.expand(C, 1, -1, -1), padding=kernel_size // 2, groups=C)
    return out
...
def train_one_epoch(model, dataloader, optimizer, lr_scheduler, scaler,
                    device, epoch, print_interval=10, wandb_writer=None, args=None, median_size=5):
    model.train()
    running_loss = 0.0
    ...
    for i, (signal, pat, _, _) in enumerate(dataloader):
        ...
        with torch.no_grad():
            if median_size is not None:
                pat = gaussian_torch(pat, sigma=median_size)
        # pat = torch.sqrt(pat)
...
@torch.no_grad()
def validate(model, val_loader, device, args, wandb_writer=None, median_size=5):
    model.eval()
    psnrs, ssims = [], []
    max_scale = args.pat_size / args.phase_size
    with torch.no_grad():
...
def main(args):
    ...
    model = OpticsViTINR(image_size=args.phase_size, patch_size=5, enc_depth=4, dec_depth=4, heads=8, dim_head=32, dim=256, 
                      mlp_dim=int(256*8/3), in_channels=1, out_channels=1, act=torch.nn.Sigmoid, out_dim=384, use_learnable_pos=False, num_reg=0, drop_path_rate=0.1, input_pad=0, pat_size=args.pat_size)
    ...
    for epoch in range(start_epoch, args.num_epochs):
        ...
        train_one_epoch(model, train_loader, optimizer, lr_scheduler, scaler, device, epoch, print_interval=args.print_interval,
                        wandb_writer=wandb_log, args=args, median_size=median_size)
        # save checkpoint
        torch.save({
            "epoch": epoch + 1,
        ...
        # validate
        psnr_mean = validate(model, val_loader, device, args=args, wandb_writer=wandb_log, median_size=median_size)
        wandb_log.log(
            {
            "epoch": epoch + 1,
            "val/psnr": psnr_mean
